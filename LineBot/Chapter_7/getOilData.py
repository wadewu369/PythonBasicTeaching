...
saveFileFormat = datetime.now().strftime("%Y%m%d")
...
def getTodayOilData():
    ...
    #     readData.loc[0:2].to_json(os.getcwd() + '/' + saveFileFormat + '_oil.json')  # 第一至三列的數據存到json檔案
    #     return True
    # else:
    #     return False
...
#     print('\n取最新的數據↓↓↓\n%s' % pd.read_json(os.getcwd() + '/' + saveFileFormat + '_oil.json'))  # 可以參考最新油價數據 Json 讀取最後儲存的油價數據
#     # return '\n取最新的數據↓↓↓\n%s' % pd.read_json(os.getcwd() + '/' + saveFileFormat + '_oil.json')
# else:
#     print("\n%s →→ 油價沒調整" % date)
#
